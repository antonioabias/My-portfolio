...
const projects = [
  {
    id: "ebuddy", num: "01", title: "E-Buddy", subtitle: "Adaptive Board Exam Reviewer",
    ...
    hasCode: true,
    gallery: [PLACEHOLDER("E-Buddy · Screenshot 1 — Replace me"), PLACEHOLDER("E-Buddy · Screenshot 2 — Replace me"), PLACEHOLDER("E-Buddy · Screenshot 3 — Replace me")],
    coverImg: "https://images.unsplash.com/photo-1434030216411-0b793f4b4173?w=800&q=80",
  },
  {
    id: "sales", num: "02", title: "Sales Information System", subtitle: "Inventory & Sales Automation",
  ...
  {
    id: "zombie", num: "03", title: "Zombie Maze Game", subtitle: "SHS Software Festival Winner",
    status: "🏆 FESTIVAL WIN", statusColor: "#FFBD2E", tags: ["Java", "Game Dev", "2017"],
    shortDesc: "2D maze survival game built solo for the SHS Software Festival.",
    fullDesc: "A 2D maze survival game designed and developed solo for the Senior High School Software Festival. Players navigate procedurally-arranged maze levels while avoiding zombies with pathfinding AI. Everything — gameplay mechanics, sprite design, and sound — was handled solo, combining technical execution with Visual Arts training.",
    highlights: ["Custom zombie pathfinding AI", "Multi-level maze progression", "Hand-drawn sprite assets", "Solo dev: design + code + audio"],
    hasCode: false,
  ...
  {
    id: "archi", num: "04", title: "AC Architectural Studio", subtitle: "Portfolio Website",
...
function ProjectModal({ project, onClose, setIsHovering }) {
  ...
  return (
    ...
    >
      ...
      >
        <div style={{ height: 4, background: project.statusColor, borderRadius: "18px 18px 0 0", opacity: 0.85 }} />
        ...
        <button
          ...
          style={{
            ...
            color: "var(--white)", width: 34, height: 34, borderRadius: "50%",
        ...
        <div className="modal-body">
          <div style={{ marginBottom: "1.75rem" }}>
            <div style={{ display: "flex", alignItems: "center", gap: "0.75rem", marginBottom: "0.5rem" }}>
              ...
              <span style={{ fontFamily: "var(--font-mono)", fontSize: "0.65rem", color: project.statusColor, border: `1px solid ${project.statusColor}44`, borderRadius: 100, padding: "0.2rem 0.65rem", letterSpacing: "0.08em" }}>{project.status}</span>
            </div>
            <h3 style={{ fontFamily: "var(--font-display)", fontSize: "clamp(1.4rem, 4vw, 2.2rem)", fontWeight: 700, letterSpacing: "-0.02em", color: "var(--white)", lineHeight: 1.1, marginBottom: "0.3rem" }}>{project.title}</h3>
            <p style={{ fontFamily: "var(--font-mono)", fontSize: "0.8rem", color: "var(--muted)" }}>{project.subtitle}</p>
          </div>

          <div style={{ marginBottom: "1.75rem" }}>
            <motion.div key={activeImg} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.25 }}
              style={{ width: "100%", aspectRatio: "16/9", borderRadius: 10, overflow: "hidden", border: "1px solid rgba(255,255,255,0.07)", marginBottom: "0.75rem", background: "#0A0F1E" }}
            >
              <img src={project.gallery[activeImg]} alt={`${project.title} screenshot ${activeImg + 1}`} style={{ width: "100%", height: "100%", objectFit: "cover" }} />
            </motion.div>
            <div style={{ display: "flex", gap: "0.5rem" }}>
              {project.gallery.map((img, i) => (
                <button key={i} onClick={() => setActiveImg(i)}
                  onMouseEnter={() => setIsHovering(true)} onMouseLeave={() => setIsHovering(false)}
                  style={{ flex: 1, aspectRatio: "16/9", borderRadius: 6, overflow: "hidden", border: i === activeImg ? "2px solid var(--cyan)" : "2px solid rgba(255,255,255,0.06)", cursor: "pointer", padding: 0, transition: "border-color 0.2s", background: "#0A0F1E" }}
                >
                  <img src={img} alt={`thumb ${i + 1}`} style={{ width: "100%", height: "100%", objectFit: "cover", opacity: i === activeImg ? 1 : 0.45, transition: "opacity 0.2s" }} />
                </button>
              ))}
            </div>
          </div>

          <div className="modal-desc-grid">
            <div>
              <div style={{ fontFamily: "var(--font-mono)", fontSize: "0.65rem", color: "var(--cyan)", letterSpacing: "0.15em", textTransform: "uppercase", marginBottom: "0.6rem", opacity: 0.7 }}>Overview</div>
              <p style={{ fontSize: "0.9rem", lineHeight: 1.75, color: "rgba(232,237,245,0.8)", margin: 0 }}>{project.fullDesc}</p>
            </div>
            <div>
              <div style={{ fontFamily: "var(--font-mono)", fontSize: "0.65rem", color: "var(--cyan)", letterSpacing: "0.15em", textTransform: "uppercase", marginBottom: "0.6rem", opacity: 0.7 }}>Key Features</div>
              <ul style={{ listStyle: "none", margin: 0, padding: 0 }}>
                {project.highlights.map((h, i) => (
                  <li key={i} style={{ display: "flex", alignItems: "flex-start", gap: "0.6rem", fontSize: "0.88rem", color: "rgba(232,237,245,0.75)", marginBottom: "0.55rem", lineHeight: 1.5 }}>
                    <span style={{ color: "var(--cyan)", flexShrink: 0, marginTop: 2 }}>▸</span>{h}
                  </li>
                ))}
              </ul>
            </div>
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "1rem", paddingTop: "1.25rem", borderTop: "1px solid rgba(255,255,255,0.06)" }}>
            ...
            {project.hasCode && (
              <a href="#contact" onClick={onClose}
                onMouseEnter={() => setIsHovering(true)} onMouseLeave={() => setIsHovering(false)}
                style={{ display: "inline-flex", alignItems: "center", gap: "0.5rem", padding: "0.6rem 1.25rem", borderRadius: 8, fontFamily: "var(--font-mono)", fontSize: "0.78rem", fontWeight: 700, letterSpacing: "0.05em", background: "var(--cyan)", color: "var(--navy)", border: "1px solid var(--cyan)", cursor: "none", transition: "opacity 0.2s" }}
              >
                Request Code Access
      ...
      <div className="projects-wrapper">
        ...
        <div className="projects-grid">
          {projects.map((project) => (
            <ProjectCard key={project.id} project={project} onClick={() => setSelected(project)} setIsHovering={setIsHovering} />
          ))}
        </div>
      </div>

      <AnimatePresence>
        {selected && <ProjectModal project={selected} onClose={() => setSelected(null)} setIsHovering={setIsHovering} />}
      </AnimatePresence>
    </section>
  );
}